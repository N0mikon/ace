...
app.whenReady().then(() => {
  ...
  electronApp.setAppUserModelId('com.ace')

  // Initialize configuration
  configManager.init()
  registerConfigIPC()

  // Initialize database and session logging
  databaseManager.init()
  registerSessionIPC()

  // Initialize agent manager
  const agentsDir = configManager.get<string>('agents.globalDirectory')
  agentManager.init(agentsDir || undefined)

  // Initialize MCP config manager
  const mcpConfigPath = configManager.get<string>('claudeCode.mcpConfig')
  mcpConfigManager.init(mcpConfigPath || undefined)
